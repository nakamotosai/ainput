...
pub struct VoiceConfig {
    ...
    pub post_commit_rewrite: PostCommitRewriteConfig,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct PostCommitRewriteConfig {
    pub enabled: bool,
    pub endpoint_url: String,
    pub model: String,
    pub api_key_env: String,
    ...
    pub require_native_replace: bool,
}
...
impl Default for VoiceConfig {
    fn default() -> Self {
        Self {
            ...
            post_commit_rewrite: PostCommitRewriteConfig::default(),
...
impl Default for PostCommitRewriteConfig {
    fn default() -> Self {
        Self {
            enabled: true,
            endpoint_url: "http://vps-jp.tail4b5213.ts.net:8317/v1/chat/completions".to_string(),
...
# 实验性：每次语音文本上屏后，后台调用 AI 改写，并尝试原位替换已上屏文本。
# 识别阶段不变；这里只处理“已经发出去的文字”。
enabled = {post_commit_rewrite_enabled}

# OpenAI 兼容改写服务地址。默认走 vps-jp cliproxyapi 8317。
endpoint_url = "{post_commit_rewrite_endpoint_url}"

# 改写模型名称。
model = "{post_commit_rewrite_model}"

# API key 环境变量名；key 不写入 TOML。
api_key_env = "{post_commit_rewrite_api_key_env}"

# 单次后台改写超时（毫秒）。
timeout_ms = {post_commit_rewrite_timeout_ms}

# 上屏后等待多久再发起后台改写（毫秒），给目标输入框一点落字时间。
delay_ms = {post_commit_rewrite_delay_ms}

# 发送给模型的光标上下文最大字符数。
max_context_chars = {post_commit_rewrite_max_context_chars}

# 允许模型返回的改写文本最大字符数。
max_output_chars = {post_commit_rewrite_max_output_chars}

# 超过这个年龄的替换票据直接放弃，避免迟到改写误伤后续输入。
max_age_ms = {post_commit_rewrite_max_age_ms}

# 后台最多同时进行多少个改写请求。
max_inflight = {post_commit_rewrite_max_inflight}

# true = 只允许 Win32/原生控件原位替换；不使用退格、删除、剪贴板或二次粘贴。
require_native_replace = {post_commit_rewrite_require_native_replace}
...
"#,
        ...
        post_commit_rewrite_enabled = config.voice.post_commit_rewrite.enabled,
        post_commit_rewrite_endpoint_url = config
            .voice
            .post_commit_rewrite
            .endpoint_url
            .replace('\\', "\\\\")
            .replace('"', "\\\""),
        post_commit_rewrite_model = config
            .voice
            .post_commit_rewrite
            .model
            .replace('\\', "\\\\")
            .replace('"', "\\\""),
        post_commit_rewrite_api_key_env = config
            .voice
            .post_commit_rewrite
            .api_key_env
            .replace('\\', "\\\\")
            .replace('"', "\\\""),
        post_commit_rewrite_timeout_ms = config.voice.post_commit_rewrite.timeout_ms,
        post_commit_rewrite_delay_ms = config.voice.post_commit_rewrite.delay_ms,
        post_commit_rewrite_max_context_chars = config.voice.post_commit_rewrite.max_context_chars,
        post_commit_rewrite_max_output_chars = config.voice.post_commit_rewrite.max_output_chars,
        post_commit_rewrite_max_age_ms = config.voice.post_commit_rewrite.max_age_ms,
        post_commit_rewrite_max_inflight = config.voice.post_commit_rewrite.max_inflight,
        post_commit_rewrite_require_native_replace =
            config.voice.post_commit_rewrite.require_native_replace,
...
"##;
    ...
    fn render_config_file_contains_streaming_ai_rewrite_section() {
        ...
        assert!(rendered.contains("[voice.post_commit_rewrite]"));
        assert!(rendered.contains("delay_ms = 160"));
        assert!(rendered.contains("require_native_replace = true"));
    ...
    #[test]
    fn post_commit_rewrite_defaults_to_aggressive_existing_route() {
        let config = PostCommitRewriteConfig::default();
        assert!(config.enabled);
        assert_eq!(
            config.endpoint_url,
            "http://vps-jp.tail4b5213.ts.net:8317/v1/chat/completions"
        ...
        assert_eq!(config.delay_ms, 160);
        assert_eq!(config.max_output_chars, 512);
        assert_eq!(config.max_inflight, 3);
        assert!(config.require_native_replace);
    }
