...
mod tests {
    ...
    fn book_json_defaults_to_pending_candidates_only() {
        let path = std::env::temp_dir().join(format!(
            "ainput-panel-book-{}-{}.json",
            std::process::id(),
            "pending-only"
        ));
        let corrections_path = std::env::temp_dir().join(format!(
            "ainput-panel-corrections-{}-{}.json",
            std::process::id(),
            "pending-only"
        ));
