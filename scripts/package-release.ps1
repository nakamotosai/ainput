...
Sync-MainConfigSectionKeys `
    ...
    -Keys @("backend", "model_dir", "sidecar_url", "sidecar_auto_start", "sidecar_idle_unload_ms", "sidecar_wsl_distro", "sidecar_wsl_workdir", "rewrite_enabled", "punctuation_model_dir", "punctuation_num_threads", "chunk_ms") `
    -ForcedValues @{
...
Set-Content -Path (Join-Path $packageDir "README.txt") -Encoding UTF8 -Value @(
    ...
    "- logs\: runtime logs",
    "- ..\..\user-voice-corpus\streaming-raw-captures\: project-level real voice corpus when running from dist",
    "",
    "Notes:",
    ...
    "- Voice modes are separate: Fast, Local Streaming, and Online Streaming; this preview defaults to Online Streaming with NVIDIA Parakeet CTC zh-CN and does not load local Qwen unless you switch modes",
    "- Real voice recordings are also saved to the project-level user-voice-corpus until it reaches 1000 wav files; after that the app skips new corpus saves instead of deleting old ones",
