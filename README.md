...
当前实测候选版本：`1.0.0-preview.84`；中文体感冻结基线仍是 `1.0.0-preview.78`（`1.0.0-preview.77` 多语言 RNNT 因中文实时识别严重漂移，已从 live 默认回滚）

本 README 是本项目唯一当前进度标准。

## 当前实测候选：1.0.0-preview.84

- 发布时间：2026-05-11。
- 目标：不改 ASR 识别链路，在已上屏文本后追加实验性 AI 改写阶段；第一版激进改写所有语音提交文本，用于测试现成 `qwen/qwen3.5-122b-a10b` 改写能力。
- 当前入口：`C:\Users\sai\ainput\dist\ainput-1.0.0-preview.84\ainput-desktop.exe`。
- 当前包：`dist\ainput-1.0.0-preview.84\` 与 `dist\ainput-1.0.0-preview.84.zip`。
...
- 当前应用侧修复：`.84` 保留 `.83` 识别链路和中英混合基准修复，新增 `[voice.post_commit_rewrite]`：已提交文本会后台送到 8317/Qwen 改写，只有捕获到原生 Edit/RichEdit 安全替换票据时才用 `EM_SETSEL + EM_REPLACESEL` 原位替换。
- 当前真实录音语料：正式录音会写入 `C:\Users\sai\ainput\user-voice-corpus\streaming-raw-captures\`，最多保存 `1000` 条；满额后跳过新增，不删除旧样本。
- 回滚点：如果 AI 改写不满意，先关闭 `voice.post_commit_rewrite.enabled=false` 或回到 `1.0.0-preview.83`；如果用户实测中文体感变差，优先回到 `1.0.0-preview.78`。

## preview.84 已上屏 AI 改写实验

- 执行合同：`specs\post-commit-ai-rewrite-preview84\SPEC.md` 和 `specs\post-commit-ai-rewrite-preview84\PLAN.md`。
- 识别阶段不变：仍使用 online streaming zh-CN Parakeet 路径，不切模型、不切部署、不启用 `multi`。
- 改写阶段：每次语音提交后异步调用 `http://vps-jp.tail4b5213.ts.net:8317/v1/chat/completions`，模型 `qwen/qwen3.5-122b-a10b`。
- 第一版策略：所有提交文本都改写，包括中文、英文、中英混合和标点；重点测试 AI 改写能力。
- 安全策略：只对 Win32 `Edit` / `RichEdit*` 控件做原位范围替换；目标文本被改动、票据超时、目标控件不支持或文本无法唯一定位时直接放弃，不使用退格、删除、剪贴板或二次粘贴。

## preview.83 中英混合 8/10 结果

- 执行合同：`specs\preview83-code-switch-8of10\SPEC.md` 和 `specs\preview83-code-switch-8of10\PLAN.md`。
- 固定基准录音：`user-voice-corpus\baselines\mixed-terms-2026-05-11-200101`，共 10 条真实录音。
- 第一门槛：已先跑 `03-codex.wav`，目标句 `让 Codex 改这里。` 已严格通过。
- 全量成功门槛：已改为 10 条全部回放、至少 8 条严格内容通过、behavior 失败为 0；不再追 9/10 或 10/10。
- 结果：`dist\ainput-1.0.0-preview.83\ainput-desktop.exe` 连续 2 轮回测均为 `8/10`，behavior failures = `0`。
- 未通过项：`JSON` 和 `Gemini`。其中 `JSON` 与普通中文 `这项` 高度歧义，本轮不做全局替换；`Gemini` 当前 ASR 原文偏离过大，不为追满分做高风险语义改写。
- 硬约束：不切模型、不切部署、不使用 `multi`；继续使用 `nvidia/parakeet-ctc-0_6b-zh-cn`、function id `9add5ef7-322e-47e0-ad7a-5653fb8d259b`、`language=zh-CN`。
- 风险边界：`把这项发给我` 与 `把 JSON 发给我` 在文本层面高度歧义；8/10 门槛下不应为了追求 9/10 或 10/10 做会伤害正常中文的全局 `这项 -> JSON` 规则。
...
- `1.0.0-preview.84` 新增第二阶段“已上屏 AI 改写”：识别与 HUD 提交不变，提交后异步改写并在安全原生控件内原位替换。
- `1.0.0-preview.83` 以用户 10 条真实中英混合短句为固定回归基准：先证明 `Codex` 单项，再以 `8/10` 作为发布门槛；JSON/Gemini 暂不为追满分做中文破坏性改写。
- `1.0.0-preview.82` 新增稳定真实录音语料库：每次正式语音录制结束后写入 `user-voice-corpus\streaming-raw-captures\`，后续 raw-corpus 测试优先从这里随机抽样。
...
- 托盘右键菜单现在会直接显示当前版本号。
- AI rewrite 不再参与 V19 识别/HUD 决策；`preview.84` 只在提交完成后作为独立异步修订器运行。
- 新增 `scripts\prune-artifacts.ps1`，可以清掉历史 `dist` / `target*` 构建垃圾，同时保留当前版本和一个回滚版本。
- 当前发包目录已经更新到 `dist\ainput-1.0.0-preview.84\` 与 `dist\ainput-1.0.0-preview.84.zip`。

它不做系统级 IME。当前默认热路径由本地 ASR/HUD 单链路负责；post-commit AI rewrite 是提交后的独立异步修订阶段。当前重点是把四条前台主链路做稳：
...
- 语音输入主链路
  ...
  - 流式模式会异步保存每次按住说话的原始录音到 `logs\streaming-raw-captures\`，自动只保留最近 `20` 组 wav + json，供短期调试使用
  - 正式语音录制还会保存到项目级 `user-voice-corpus\streaming-raw-captures\`，供后续真实录音回归测试随机抽样；该目录最多 `1000` 条 wav，满额后只跳过新增，不自动删除
  - 语音热键可配置
  - 自动直贴失败时，可按配置降级到剪贴板
  - 普通输入框与终端输入区使用不同句号策略
- 截图主链路
...
- 当前可直接实测的便携版是 `dist\ainput-1.0.0-preview.84\`
- 默认启动模式是 `在线流式识别`
...
- 默认热路径由本地 ASR/HUD 单链路负责；V19 不跑 release-time offline final，不做 HUD/offline 尾巴合并，不在松手前二次修正 HUD 文本
- post-commit AI rewrite 是提交后的独立异步阶段；它不会参与识别、HUD truth 或提交文本选择
- `preview.84` 是在 `preview.83` 上追加已上屏 AI 改写实验的版本；`preview.83` 是 10 条中英混合基准 8/10 版本；`preview.82` 是真实录音自动保存版本；`preview.78` 是中文体感冻结基线和快速回滚点；`preview.77` 是失败的多语言 RNNT 实验包；`preview.76` 是中文 CTC 在线模式回滚点；`preview.72` 仍是本机 Qwen 回滚点
- `preview.71` 会泄漏 Qwen context 到 HUD，不建议作为回滚点；需要回滚时优先回到最后一个已验证可接受的旧包并重新验证真实麦克风链路
- 收口门禁脚本是 `scripts\readme_closeout_guard.py`

## Roadmap / Future Work

- AI rewrite 已按独立 post-commit spec 回到实验版；后续应继续保持“识别/HUD 与提交后修订”两阶段隔离。
...
2026-05-11 preview.84 post-commit AI rewrite experiment:

- 目标：识别链不动，新增提交后的异步 AI 改写和安全原位替换。
- 修复：新增 `[voice.post_commit_rewrite]`，使用 8317/Qwen 现成路由；worker 在 fast / streaming / sidecar fast / Qwen final commit 成功后调度后台改写。
- 安全边界：只捕获原生 `Edit` / `RichEdit*` 控件范围票据；替换前复核窗口、class、前缀和原文 range，失败则无声放弃。
- 验证：见本节后续 preview84 打包与运行记录。

2026-05-11 preview.83 mixed Chinese/English baseline:

- 目标：不修改模型、部署、function id、`language=zh-CN` 或启用 `multi`，只在当前 zh-CN Parakeet 链路上做保守应用侧修复。
- 门槛：先跑 `03-codex.wav` 单项；如果 Codex 3 轮内不能修好就停止；Codex 通过后才跑全量 10 条，发布门槛为至少 `8/10` 严格内容通过且 behavior failures = `0`。
- 修复：新增 `scripts\run-mixed-terms-baseline.ps1`，并在 `ainput-rewrite` / worker 最终文本路径加入窄范围 mixed-term repair。
- 覆盖：HUD、Codex、Claude Code、OpenAI、GitHub、CPA、token、VPS 通过固定基准；JSON 和 Gemini 保留为已知失败项。
- 验证：`dist\ainput-1.0.0-preview.83\ainput-desktop.exe -OnlyTerm Codex -MinPass 1` passed，1/1，behavior failures = `0`。
- 验证：`dist\ainput-1.0.0-preview.83\ainput-desktop.exe -MinPass 8 -Repeat 2` passed，连续两轮均为 `8/10`，behavior failures = `0`。
- 验证：Windows 交互桌面进程已运行 `dist\ainput-1.0.0-preview.83\ainput-desktop.exe`，`SessionId=1`；`run-ainput.bat` 与 HKCU Run 均指向 preview.83。
- 未覆盖：用户自由口述的主观中文体感仍以后续真实使用为准；本轮只证明固定 10 条真实录音基准达到 8/10。

2026-05-11 preview.82 real user voice corpus:

- 目标：不修改模型、部署、function id、`language=zh-CN` 或 `.81` 的 code-switch 后处理，只新增用户真实录音自动留样。
- 修复：流式热键、sidecar fast commit、极速语音热键和 `record-once` 录音结束后会写入项目级 `user-voice-corpus\streaming-raw-captures\`。
- 保护：现有 `logs\streaming-raw-captures\` 仍是短期调试目录，只保留最近 `20` 组 wav + json；长期用户语料目录最多 `1000` 条 wav，满额后跳过新增，不清理旧样本。
- 工具：`scripts\run-streaming-raw-corpus.ps1` 默认优先选择 `user-voice-corpus\streaming-raw-captures\`；使用该目录时默认随机抽样，可用 `-Deterministic` 回到短/长样本选择。
- 验证：`cargo fmt --all -- --check` passed；`cargo test -p ainput-desktop raw_capture -- --nocapture` 2/2 passed；`cargo test -p ainput-desktop user_voice_corpus -- --nocapture` 3/3 passed；`git diff --check` passed；`python .\scripts\readme_closeout_guard.py .` passed。
- 验证：`scripts\package-release.ps1 -Version 1.0.0-preview.82` 已生成 dist 与 zip；Windows 交互桌面已运行 `dist\ainput-1.0.0-preview.82\ainput-desktop.exe`，`SessionId=1`；`run-ainput.bat` 与 HKCU Run 均指向 preview.82。
- 验证：包内 `record-once 1` 实测写入 `user-voice-corpus\streaming-raw-captures\`，当前语料目录已有 `1` 组 wav + json，最新 wav `94124` bytes。
- 验证：`scripts\run-streaming-raw-corpus.ps1` 选择逻辑命中 `raw_dir_source=user_voice_corpus`、`selection_mode=random`、`cases_total=1`。
- 未覆盖：用户自由口述的主观识别质量仍以后续真实使用为准；`.82` 只负责沉淀真实录音，不改变识别策略。
...
- 当前运行：`1.0.0-preview.84` 已启动到 Windows 交互桌面，作为已上屏 AI 改写实验候选。
- 当前冻结：`1.0.0-preview.78` 已在 2026-05-11 经用户真实使用确认，仍是中文体感冻结基线和快速回滚点。
- 当前基线：识别速度快、HUD 实时、松手上屏快；后续改动不得用未实测方案覆盖这条已确认链路。
- 当前进度：`1.0.0-preview.84` 是独立在线流式 ASR 默认版加 post-commit AI rewrite 实验版，默认模式仍为 `online_streaming`，沿用 zh-CN CTC 当前模型路径。
- 当前入口：`C:\Users\sai\ainput\dist\ainput-1.0.0-preview.84\ainput-desktop.exe`。
- 当前包：`dist\ainput-1.0.0-preview.84\` 与 `dist\ainput-1.0.0-preview.84.zip`。
- 当前真实录音语料：`C:\Users\sai\ainput\user-voice-corpus\streaming-raw-captures\`，最多 `1000` 条 wav，后续测试优先从这里随机抽样。
...
- preview.84 没有切 ASR 模型和部署；沿用当前 zh-CN Parakeet 路径，在 preview.83 基础上只新增提交后 AI 改写实验
- 默认流式 ASR 依赖 NVIDIA 在线服务；post-commit AI rewrite 依赖 vps-jp 8317/Qwen 现成路由
- AI 语义改写只允许改已上屏且被安全票据锁定的文本；当前上屏文本仍先来自 HUD truth snapshot
...
- 应用层 streaming HUD AI rewrite 仍关闭；post-commit rewrite 可单独开关，不能绕过 HUD truth，只能改已经上屏且被安全票据锁定的文本。
