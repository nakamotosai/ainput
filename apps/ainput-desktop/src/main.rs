...
fn try_main() -> Result<()> {
    ...
    if args.get(1).map(String::as_str) == Some("test-post-commit-ai-rewrite") {
        let runtime = build_runtime(&bootstrap, false)?;
        let committed_text = args.get(2).ok_or_else(|| {
            anyhow!("usage: ainput-desktop test-post-commit-ai-rewrite <committed-text>")
        })?;
        let ai_rewriter = runtime
            .post_commit_rewriter
            .as_ref()
            .ok_or_else(|| anyhow!("post-commit AI rewrite client is not enabled"))?;
        let response =
            ai_rewriter.rewrite_committed_text(ai_rewrite::PostCommitRewriteRequest {
        ...
        if let Some(response) = response {
            println!("{}", response.replacement_text);
        }
        return Ok(());
    }
...
fn build_runtime(
    ...
    let post_commit_rewriter =
        try_prepare_post_commit_rewriter(&bootstrap.config.voice.post_commit_rewrite);
...
fn try_prepare_post_commit_rewriter(
    ai_config: &ainput_shell::PostCommitRewriteConfig,
) -> Option<Arc<ai_rewrite::AiRewriteClient>> {
    if !ai_config.enabled {
        return None;
    }

    match ai_rewrite::AiRewriteClient::from_post_commit_config(ai_config) {
        Ok(Some(ai_rewriter)) => {
            let ai_rewriter = Arc::new(ai_rewriter);
            tracing::info!(
                endpoint_url = %ai_config.endpoint_url,
                model = %ai_config.model,
                "post-commit AI rewrite client enabled"
            );
            let prewarm_client = Arc::clone(&ai_rewriter);
            thread::spawn(move || {
                if let Err(error) = prewarm_client.prewarm_post_commit() {
                    tracing::warn!(error = %error, "post-commit AI rewrite warmup failed");
                }
            });
            Some(ai_rewriter)
        }
        ...
        Err(error) => {
            tracing::warn!(
                error = %error,
                "failed to initialize post-commit AI rewrite client; keeping committed text unchanged"
            );
            None
        }
    }
...
pub(crate) struct AppRuntime {
    ...
    post_commit_rewriter: Option<Arc<ai_rewrite::AiRewriteClient>>,
