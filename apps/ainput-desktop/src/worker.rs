...
use crate::ai_rewrite::{AiRewriteRequest, PostCommitRewriteRequest};
...
const STREAMING_RELEASE_MIN_WAIT_MS: u64 = 120;
const STREAMING_RELEASE_IDLE_SETTLE_MS: u64 = 120;
const STREAMING_RELEASE_POLL_INTERVAL_MS: u64 = 8;
const STREAMING_IDLE_FINALIZE_TAIL_PADDING_MS: u64 = 320;
const STREAMING_HUD_SOFT_FLUSH_MS: u64 = 240;
const STREAMING_HUD_SOFT_FLUSH_MIN_VISIBLE_CHARS: usize = 4;
const STREAMING_HUD_SOFT_FLUSH_TAIL_PADDING_MS: u64 = 180;
...
static POST_COMMIT_REWRITE_INFLIGHT: AtomicUsize = AtomicUsize::new(0);
...
struct PendingQwenSessionStart {
    rx: mpsc::Receiver<Result<(QwenStartResponse, bool), String>>,
    hold_active: bool,
}

struct PendingQwenModelPreload {
    rx: mpsc::Receiver<Result<(), String>>,
    start_session_after_ready: bool,
}
...
pub(crate) struct StreamingLiveProbeReport {
    ...
    hud_truth_text: String,
    pasted_text: String,
    hud_paste_equal: bool,
    offline_final_invoked: bool,
    ai_rewrite_mutation_count: usize,
    final_text: String,
...
impl QwenSidecarClient {
    fn create(config: &ainput_shell::StreamingVoiceConfig) -> Result<Self> {
        let http = Client::builder()
            .timeout(QWEN_SIDECAR_HTTP_TIMEOUT)
            .no_proxy()
            .build()
            .context("build Qwen3-ASR sidecar HTTP client")?;
        let base_url = config.sidecar_url.trim().trim_end_matches('/').to_string();
        if base_url.is_empty() {
            bail!("voice.streaming.sidecar_url is empty");
        }
        let _ = config;
        Ok(Self { http, base_url })
    }

    fn health(&self) -> Result<QwenHealthResponse> {
        let response = self
            .http
            .get(format!("{}/health", self.base_url))
            .send()
            .context("call Qwen3-ASR sidecar health")?
            .error_for_status()
            .context("Qwen3-ASR sidecar health returned error")?
            .json::<QwenHealthResponse>()
            .context("decode Qwen3-ASR sidecar health")?;
        if !response.ok {
            bail!("Qwen3-ASR sidecar health ok=false");
        }
        Ok(response)
    }
    ...
    fn start_session(
        ...
        self.http
            ...
            .error_for_status()
            .context("Qwen3-ASR sidecar session create returned error")?
            .json::<QwenStartResponse>()
            .context("decode Qwen3-ASR sidecar session create")
    }

    fn start_session_with_recovery(
        &self,
        config: &ainput_shell::StreamingVoiceConfig,
        ...
        match self.start_session(config) {
            ...
            Err(first_error) => {
                tracing::warn!(
                    error = %first_error,
                    sidecar_url = %self.base_url,
                    "Qwen3-ASR sidecar session create failed; ensuring sidecar is ready and retrying"
                );
    ...
    fn preload_model_with_warm_session(
        &self,
        config: &ainput_shell::StreamingVoiceConfig,
    ) -> Result<()> {
        self.ensure_ready(config)?;
        ...
        let sample_rate_hz = self
            .health()
            .ok()
            .and_then(|health| health.sample_rate_hz)
            .unwrap_or(16_000)
            .max(8_000) as usize;
        ...
        let warm_chunk = vec![0.0f32; warm_chunk_samples];
        let _ = self
            .send_chunk(&warm_session.session_id, &warm_chunk)
            .context("send Qwen3-ASR warm preload chunk")?;
        self.finish_session(&warm_session.session_id)
            .context("finish Qwen3-ASR warm preload session")?;
        tracing::info!(
            sample_rate_hz,
            warm_chunk_samples,
            warm_chunk_ms = QWEN_PRELOAD_WARM_CHUNK_MS,
            "Qwen3-ASR preload warm chunk completed"
        );
        Ok(())
    }

    fn send_chunk(&self, session_id: &str, samples: &[f32]) -> Result<QwenChunkResponse> {
        let body = f32_pcm_le_bytes(samples);
        self.http
            .post(format!("{}/v1/sessions/{session_id}/chunk", self.base_url))
            .header("Content-Type", "application/octet-stream")
            .body(body)
            .send()
            .with_context(|| format!("send Qwen3-ASR sidecar chunk for {session_id}"))?
            .error_for_status()
            .context("Qwen3-ASR sidecar chunk returned error")?
            .json::<QwenChunkResponse>()
            .context("decode Qwen3-ASR sidecar chunk response")
    }

    fn finish_session(&self, session_id: &str) -> Result<QwenFinishResponse> {
        self.http
            .post(format!("{}/v1/sessions/{session_id}/finish", self.base_url))
            .send()
            .with_context(|| format!("finish Qwen3-ASR sidecar session {session_id}"))?
            .error_for_status()
            .context("Qwen3-ASR sidecar finish returned error")?
            .json::<QwenFinishResponse>()
            .context("decode Qwen3-ASR sidecar finish response")
    }
}

fn f32_pcm_le_bytes(samples: &[f32]) -> Vec<u8> {
...
fn start_qwen_sidecar_via_wsl(config: &ainput_shell::StreamingVoiceConfig) -> Result<()> {
    let workdir = config.sidecar_wsl_workdir.trim();
    if workdir.is_empty() {
...
fn spawn_qwen_sidecar_session_start(
    config: ainput_shell::StreamingVoiceConfig,
) -> mpsc::Receiver<Result<(QwenStartResponse, bool), String>> {
    let (tx, rx) = mpsc::channel();
    std::thread::spawn(move || {
        let result = match QwenSidecarClient::create(&config) {
            Ok(client) => client
                .start_session_with_recovery(&config)
...
fn spawn_qwen_sidecar_model_preload(
    config: ainput_shell::StreamingVoiceConfig,
    ...
    std::thread::spawn(move || {
        let result = match QwenSidecarClient::create(&config) {
            Ok(client) => client
                .preload_model_with_warm_session(&config)
                .map_err(|error| error.to_string()),
            Err(error) => Err(error.to_string()),
        };
        let _ = tx.send(result);
    ...
    let recognizer = match build_recognizer(&runtime) {
        ...
        Err(error) => {
            let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::ModelLoadFailed {
                kind: WorkerKind::Fast,
                message: format!("初始化识别器失败：{error}"),
            }));
            return;
        }
    };
                        ...
                        match ainput_audio::ActiveRecording::start_default_input() {
                            ...
                            Err(error) => {
                                ...
                                let _ = proxy.send_event(AppEvent::Worker(
                                    WorkerEvent::RecoverableError(format!("启动录音失败：{error}")),
                                ));
                            }
                        }
                    }
...
pub(crate) fn streaming_push_to_talk_worker(
    ...
    if !backend.is_empty() && backend != "sherpa" {
        let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::Error(format!(
            "unknown streaming ASR backend: {}",
            runtime.config.voice.streaming.backend
        ))));
        return;
    }
    ...
    let recognizer = match build_streaming_recognizer(&runtime) {
        ...
        Err(error) => {
            let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::ModelLoadFailed {
                kind: WorkerKind::Streaming,
                message: format!("初始化流式识别器失败：{error}"),
            }));
            return;
        }
    };
    ...
    tracing::info!("streaming offline final repair disabled for HUD truth single-chain v19");
            ...
            match command {
                ...
                WorkerCommand::HotkeyPressed => {
                    if active_session.is_none() {
                        if let Err(error) =
                            ensure_streaming_recording_ready(&runtime, &mut standby_recording)
                        {
                            tracing::error!(
                                error = %error,
                            ...
                            let _ = proxy.send_event(AppEvent::Worker(
                                WorkerEvent::RecoverableError(format!("启动流式录音失败：{error}")),
                            ));
                            continue;
                        }
                        ...
                        if let Some(recording) = standby_recording.as_ref() {
                            ...
                            tracing::info!(
                                ...
                                input_channels = recording.channels(),
                                asr_sample_rate_hz = runtime.config.asr.sample_rate_hz,
                                asr_channels = 1,
                                direct_asr_format = recording.sample_rate_hz()
                                    == runtime.config.asr.sample_rate_hz as i32
                                    && recording.channels() == 1,
                                current_cursor,
                                start_cursor,
                                preroll_ms,
                                "streaming push-to-talk recording started with v19 ASR-facing mono stream"
                            );
                        }
                    }
                }
                WorkerCommand::HotkeyReleased => {
                    if let Some(mut session) = active_session.take() {
                        ...
                        let had_partial_truth_before_finish = session.partial_updates > 0
                            || !session.last_display_text.trim().is_empty();
                        if session.captured_samples.is_empty()
                            || (should_skip_as_silence(&activity)
                                && !had_partial_truth_before_finish)
                        {
                            tracing::info!(
                                audio_duration_ms = captured_audio_duration_ms,
                                peak_abs = format_args!("{:.6}", activity.peak_abs),
                        ...
                        let hud_paste_equal = hud_commit_text == commit_text;
                        ...
                        tracing::info!(
                            ...
                            hud_paste_equal,
                            offline_final_invoked = false,
                            ai_rewrite_mutation_count = session.post_hud_flush_mutation_count,
...
fn sidecar_streaming_push_to_talk_worker(
    ...
    let mut standby_recording: Option<ainput_audio::ActiveRecording> = None;
    let mut active_session: Option<QwenSidecarSession> = None;
    let mut pending_model_preload: Option<PendingQwenModelPreload> = None;
    let mut pending_session_start: Option<PendingQwenSessionStart> = None;

    while !shutdown.load(Ordering::Relaxed) {
        if let Ok(command) = worker_rx.recv_timeout(Duration::from_millis(8)) {
            match command {
                WorkerCommand::PreloadModel => {
                    if !sidecar_model_ready
                        && active_session.is_none()
                        && pending_session_start.is_none()
                        && pending_model_preload.is_none()
                    {
                        tracing::info!(
                            sidecar_url = %runtime.config.voice.streaming.sidecar_url,
                            backend = backend_label,
                        ...
                        pending_model_preload = Some(PendingQwenModelPreload {
                            rx: spawn_qwen_sidecar_model_preload(
                                runtime.config.voice.streaming.clone(),
                            ),
                            start_session_after_ready: false,
                        });
                    }
                }
                WorkerCommand::HotkeyPressed => {
                    if active_session.is_none() {
                        if sidecar_model_ready && pending_session_start.is_none() {
                        ...
                        } else if let Some(pending) = &mut pending_session_start {
                            pending.hold_active = true;
                        } else if let Some(pending) = &mut pending_model_preload {
                            pending.start_session_after_ready = true;
                        } else {
                            tracing::info!(
                                sidecar_url = %runtime.config.voice.streaming.sidecar_url,
                                backend = backend_label,
                            ...
                            pending_model_preload = Some(PendingQwenModelPreload {
                                ...
                                start_session_after_ready: true,
                            });
                        }
                    }
                }
                WorkerCommand::HotkeyReleased => {
                    if active_session.is_none() {
                        if let Some(pending) = &mut pending_session_start {
                            pending.hold_active = false;
                            tracing::info!(
                                "voice hotkey released while Qwen sidecar session bootstrap was still pending"
                            );
                            let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::IgnoredSilence));
                        } else if let Some(pending) = &mut pending_model_preload {
                            let had_waiting_hold = pending.start_session_after_ready;
                            pending.start_session_after_ready = false;
                            if had_waiting_hold {
                                tracing::info!(
                                    "voice hotkey released while Qwen sidecar model preload was still pending"
                                );
                                let _ =
                    ...
                    if let Some(mut session) = active_session.take() {
                        let Some(recording) = standby_recording.take() else {
                            tracing::error!("Qwen streaming recording disappeared before release");
                            let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::Error(
                                "Qwen streaming recording state was lost; retry".to_string(),
                            )));
                            continue;
                        };
                        let release_started_at = Instant::now();
                        ...
                        let release_drain = match finish_qwen_streaming_recording(
                            ...
                            Ok(stats) => stats,
                            Err(error) => {
                                tracing::error!(error = %error, "failed to finalize Qwen streaming recording");
                        ...
                        let streamed_samples = match feed_qwen_pending_chunks(
                            &sidecar,
                            &mut session,
                            chunk_samples,
                            false,
                            ...
                            Ok(samples) => samples,
                            Err(error) => {
                                tracing::error!(error = %error, "failed to feed final Qwen sidecar chunks");
                                sidecar_model_ready = false;
                                let _ = proxy.send_event(AppEvent::Worker(
                                    WorkerEvent::ModelLoadFailed {
                                        kind: mode.kind,
                        ...
                        let captured_audio_duration_ms = audio_duration_ms(
                            session.sample_rate_hz,
                        ...
                        tracing::info!(
                            ...
                            release_stop_added_samples = release_drain.stop_added_samples,
                            release_grace_wait_elapsed_ms = release_drain.grace_wait_elapsed_ms,
                            release_voice_active_observations = release_drain.voice_active_observations,
                            release_tail_timeout_fallback = release_drain.timeout_fallback,
                            streamed_samples,
                            total_chunks_fed = session.total_chunks_fed,
                        ...
                        let had_partial_truth_before_finish =
                                ...
                                && !had_partial_truth_before_finish)
                        {
                            tracing::info!(
                                ...
                                active_ratio = format_args!("{:.4}", activity.active_ratio),
                                sustained_voice_ms = activity.sustained_voice_ms,
                                "skip Qwen streaming transcription because captured audio looks like silence"
                            );
                            let _ = sidecar.finish_session(&session.session_id);
                            let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::IgnoredSilence));
                            continue;
                        }
                        if should_skip_as_silence(&activity) && had_partial_truth_before_finish {
                            tracing::info!(
                                session_id = %session.session_id,
                                partial_updates = session.partial_updates,
                                last_display_text = %short_log_text(&session.last_display_text, 120),
                                audio_duration_ms = captured_audio_duration_ms,
                                peak_abs = format_args!("{:.6}", activity.peak_abs),
                                rms = format_args!("{:.6}", activity.rms),
                                active_ratio = format_args!("{:.4}", activity.active_ratio),
                                sustained_voice_ms = activity.sustained_voice_ms,
                                "bypass Qwen low-signal silence gate because HUD truth already exists"
                            );
                        }

                        let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::StreamingFlushing));
                        let finish_started_at = Instant::now();
                        let finish = match sidecar.finish_session(&session.session_id) {
                            Ok(response) => response,
                            Err(error) => {
                                tracing::error!(error = %error, "failed to finish Qwen sidecar session");
                                sidecar_model_ready = false;
                                let _ = proxy.send_event(AppEvent::Worker(
                                    WorkerEvent::ModelLoadFailed {
                                        kind: mode.kind,
                            ...
                            || should_drop_low_signal_result(&final_text, &activity)
                        {
                            if had_partial_truth_after_finish {
                                ...
                                if !fallback_text.is_empty() {
                                    tracing::info!(
                                        session_id = %session.session_id,
                                        qwen_final_raw_text = %finish.text,
                                        final_text = %final_text,
                                        fallback_text = %fallback_text,
                                        sidecar_audio_ms = finish.audio_ms,
                                        sidecar_elapsed_ms = finish.elapsed_ms,
                                        sidecar_finished = finish.finished,
                                        "use existing Qwen HUD truth because final decode was empty or low-signal"
                                    );
                                    final_text = fallback_text;
                                }
                            }
                            ...
                            || should_drop_low_signal_result(&final_text, &activity)
                        {
                            tracing::info!(
                                ...
                                sidecar_elapsed_ms = finish.elapsed_ms,
                                sidecar_finished = finish.finished,
                                "drop empty or low-signal Qwen streaming final text"
                            );
                            let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::IgnoredSilence));
                            continue;
                        }
                        ...
                        let hud_ack = if runtime.config.voice.streaming.panel_enabled
                            && runtime
                                ...
                                .require_hud_flush_before_commit
                        {
                            match request_streaming_final_hud_commit_ack(&proxy, &final_text) {
                                Ok(ack) => ack,
                                Err(error) => {
                                    tracing::error!(
                                        error = %error,
                                        commit_text = %short_log_text(&final_text, 120),
                                        "Qwen final HUD commit ack failed; skip paste"
                                    );
                                    let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::Error(
                                        format!(
                                            "final HUD commit ack failed; paste canceled: {error}"
                                        ),
                                    )));
                                    continue;
                                }
                        ...
                        let hud_commit_text = hud_ack.text.trim().to_string();
                        if hud_commit_text.is_empty() {
                            tracing::error!("Qwen final HUD commit ack returned empty text");
                            let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::Error(
                                "final HUD commit text was empty; paste canceled".to_string(),
                            )));
                            continue;
                        }
                        let hud_paste_equal = hud_commit_text == final_text;
                        let hud_final_flush_elapsed_ms =
                            hud_final_flush_started_at.elapsed().as_millis();
                        tracing::info!(
                            session_id = %session.session_id,
                            qwen_final_raw_text = %finish.text,
                            qwen_language = %finish.language.as_deref().unwrap_or("unknown"),
                            final_text = %final_text,
                            hud_commit_text = %hud_commit_text,
                            hud_paste_equal,
                            offline_final_invoked = false,
                            ...
                            hud_final_flush_elapsed_ms,
                            total_chunks_fed = session.total_chunks_fed,
                            "Qwen sidecar final transcription ready"
                        );
                        ...
                        if !modifiers_released {
                            tracing::warn!(
                                waited_ms = hotkey_release_wait_elapsed_ms,
                                streaming_effective_hotkey = "Ctrl",
                                "Qwen streaming output started before all modifiers fully released"
                            );
                        }
                        ...
                        tracing::info!(
                            ...
                            output_elapsed_ms,
                            release_to_commit_elapsed_ms,
                            "Qwen sidecar streaming transcription delivered"
                        );
                        let drained_commands = drain_pending_voice_commands(&worker_rx);
                        if drained_commands > 0 {
                            tracing::warn!(
                                drained_commands,
                                "dropped queued voice hotkey commands after Qwen streaming commit"
                            );
                        }
                        let _ = proxy.send_event(AppEvent::Worker(WorkerEvent::StreamingFinal(
                            hud_commit_text,
        ...
        if active_session.is_none() {
            let preload_result = if let Some(pending) = &mut pending_model_preload {
                match pending.rx.try_recv() {
                    Ok(result) => Some((pending.start_session_after_ready, result)),
                    Err(mpsc::TryRecvError::Empty) => None,
                    Err(mpsc::TryRecvError::Disconnected) => Some((
                        pending.start_session_after_ready,
                        Err("Qwen sidecar preload thread disconnected".to_string()),
                    )),
                }
            } else {
                None
            ...
            if let Some((start_session_after_ready, result)) = preload_result {
                pending_model_preload = None;
                match result {
                    Ok(()) => {
                        ...
                        if start_session_after_ready && pending_session_start.is_none() {
                            tracing::info!(
                                sidecar_url = %runtime.config.voice.streaming.sidecar_url,
                                "Qwen sidecar preload finished; starting deferred session bootstrap"
                            );
                            pending_session_start = Some(PendingQwenSessionStart {
                                rx: spawn_qwen_sidecar_session_start(
                                    runtime.config.voice.streaming.clone(),
            ...
            let startup_result = if let Some(pending) = &mut pending_session_start {
                match pending.rx.try_recv() {
                    Ok(result) => Some((pending.hold_active, result)),
                    Err(mpsc::TryRecvError::Empty) => None,
                    Err(mpsc::TryRecvError::Disconnected) => Some((
                        pending.hold_active,
                        Err("Qwen sidecar bootstrap thread disconnected".to_string()),
                    )),
                }
            } else {
                None
            ...
            if let Some((hold_active, result)) = startup_result {
                    ...
                    Ok((sidecar_session, sidecar_restarted)) => {
                        ...
                        if !hold_active {
                            tracing::info!(
                                ...
                                "Qwen sidecar session became ready after key release; closing unused session"
                            );
                            let _ = sidecar.finish_session(&sidecar_session.session_id);
                        } else {
                            if let Err(error) =
                                ensure_streaming_recording_ready(&runtime, &mut standby_recording)
                            {
                                tracing::error!(error = %error, "failed to start Qwen streaming microphone recording after sidecar became ready");
                                let _ = proxy.send_event(AppEvent::Worker(
                                    WorkerEvent::RecoverableError(format!(
                                        "failed to start Qwen streaming recording: {error}"
                                    )),
                                ));
                                continue;
                            }
                            ...
                            let preroll_samples =
                                sample_count_for_ms(recording.sample_rate_hz(), preroll_ms);
                            let start_cursor = if sidecar_restarted {
                                0
                            } else {
                                current_cursor.saturating_sub(preroll_samples)
                            };
                            let session = QwenSidecarSession::new(
                                recording.sample_rate_hz(),
                                sidecar_session,
                                start_cursor,
                            )
                            ...
                            tracing::info!(
                                ...
                                input_sample_rate_hz = recording.sample_rate_hz(),
                                input_channels = recording.channels(),
                                asr_sample_rate_hz = session.sample_rate_hz,
                                asr_channels = 1,
                                direct_asr_format = recording.sample_rate_hz() == session.sample_rate_hz
                                    && recording.channels() == 1,
                                sidecar_restarted,
                                current_cursor,
                                start_cursor,
                                preroll_ms,
                                "Qwen sidecar streaming recording started with V19 HUD truth chain"
                            );
                ...
                Ok(streamed_samples) => {
                    if streamed_samples > 0 || added_samples > 0 {
                        tracing::trace!(
                            ...
                            streamed_samples,
                            "Qwen sidecar keeps one continuous streaming session; no offline final merge"
                        );
                    }
                }
                Err(error) => {
...
fn collect_qwen_audio_chunk(
    ...
    push_qwen_input_samples(session, &chunk)
}

fn push_qwen_input_samples(session: &mut QwenSidecarSession, input: &[f32]) -> usize {
    if input.is_empty() {
        return 0;
    }
...
fn flush_qwen_audio_tail(session: &mut QwenSidecarSession) -> usize {
...
fn collect_qwen_stopped_recording_tail(
    ...
    push_qwen_input_samples(session, tail)
}

fn finish_qwen_streaming_recording(
    session: &mut QwenSidecarSession,
    recording: ainput_audio::ActiveRecording,
    raw_capture_dir: PathBuf,
    ...
    loop {
        let added = collect_qwen_audio_chunk(session, &recording);
    ...
    let stop_added_samples = collect_qwen_stopped_recording_tail(session, &recorded);
    save_streaming_raw_capture_async(raw_capture_dir, recorded);
    let _ = flush_qwen_audio_tail(session);
    Ok(StreamingReleaseDrainStats {
        grace_added_samples,
        stop_added_samples,
        grace_wait_elapsed_ms: grace_started_at.elapsed().as_millis(),
        voice_active_observations,
        timeout_fallback,
    })
}

fn feed_qwen_pending_chunks(
    sidecar: &QwenSidecarClient,
    session: &mut QwenSidecarSession,
...
pub(crate) fn probe_streaming_live_session(
    ...
    let recording = ainput_audio::ActiveRecording::start_default_input_for_asr(
        runtime.config.asr.sample_rate_hz as i32,
        1,
    )?;
    ...
    while Instant::now() < deadline {
        ...
        if streamed_samples > 0 || added_samples > 0 {
            ...
            tracing::trace!(
                total_audio_duration_ms,
                "v19 live probe bypassed endpoint rollover to keep HUD truth on one streaming chain"
            );
        }
    ...
    Ok(StreamingLiveProbeReport {
        ...
        hud_truth_text: final_text.clone(),
        pasted_text: final_text.clone(),
        hud_paste_equal: true,
        offline_final_invoked: false,
        ai_rewrite_mutation_count: session.post_hud_flush_mutation_count,
        final_text,
...
pub(crate) fn replay_streaming_wav(
    ...
    for chunk in input_samples.chunks(replay_chunk_num_samples.max(1)) {
        ...
        tracing::trace!(
            total_audio_duration_ms,
            "v19 replay bypassed pause soft-flush and endpoint rollover to keep one streaming chain"
        );
    }
...
fn ensure_streaming_recording_ready(
    ...
    if standby_recording.is_none() {
        let target_sample_rate_hz = runtime.config.asr.sample_rate_hz as i32;
        let recording =
            ainput_audio::ActiveRecording::start_default_input_for_asr(target_sample_rate_hz, 1)?;
        tracing::info!(
            input_sample_rate_hz = recording.sample_rate_hz(),
            input_channels = recording.channels(),
            asr_sample_rate_hz = target_sample_rate_hz,
            asr_channels = 1,
            direct_asr_format =
                recording.sample_rate_hz() == target_sample_rate_hz && recording.channels() == 1,
            "streaming microphone armed on hotkey press for v19"
        );
...
fn prepare_final_streaming_commit(
    ...
    let final_offline_raw_text = String::new();
    let offline_final_elapsed_ms = 0;
    let offline_final_timed_out = false;
    ...
    let (candidate_display_text, rejected_prefix_rewrite) = if prepared_final_candidate
        .trim()
        .is_empty()
    {
        (display_text_before_final.clone(), false)
    } else {
        let delta = session
            .state
            .finalize_from_streaming(&prepared_final_candidate);
        if delta.rejected_prefix_rewrite {
            tracing::warn!(
                final_online_raw_text = %short_log_text(&final_online_raw_text, 120),
                prepared_final_candidate = %short_log_text(&prepared_final_candidate, 120),
                display_text_before_final = %short_log_text(&display_text_before_final, 120),
                "streaming final ASR candidate rejected by HUD truth state; keeping current HUD text"
            );
            (display_text_before_final.clone(), true)
        } else {
            let hud_truth_text = session.state.full_text().trim().to_string();
            session.last_raw_partial = final_online_raw_text.clone();
            session.last_display_text = hud_truth_text.clone();
            session.last_fast_preview_text = hud_truth_text.clone();
            (hud_truth_text, false)
        }
    };

    let selected_commit_text = if candidate_display_text.trim().is_empty() {
        display_text_before_final.trim()
    } else {
        candidate_display_text.trim()
    };
    let final_text = finalize_streaming_commit_text(selected_commit_text);
    let display_text_trimmed = display_text_before_final.trim();
    let commit_source = if rejected_prefix_rewrite || final_text == display_text_trimmed {
        StreamingCommitSource::StreamingState
    } else if final_text != selected_commit_text {
    ...
    tracing::info!(
        ...
        final_text = %short_log_text(&final_text, 120),
        final_decode_steps,
        online_final_elapsed_ms,
        offline_final_invoked = false,
        "streaming HUD truth final prepared without offline final"
    );
...
fn resolve_final_streaming_commit(
    ...
    let (candidate_display_text, rejected_prefix_rewrite) = if candidate_trimmed.is_empty() {
        (String::new(), false)
    } else if matches!(candidate_source, StreamingCommitSource::OfflineFinal)
        && !display_trimmed.is_empty()
        && !candidate_trimmed.starts_with(&display_trimmed)
        && let Some((repaired, overlap_chars)) = repair_final_candidate_tail_overlap(
            &display_trimmed,
            &format!("{display_trimmed}{candidate_trimmed}"),
        )
    {
        tracing::info!(
            overlap_chars,
            display_text = %short_log_text(&display_trimmed, 120),
            offline_tail = %short_log_text(candidate_trimmed, 120),
            repaired_text = %short_log_text(&repaired, 120),
            "streaming final commit repaired offline tail before state merge"
        );
        (repaired, false)
    } else {
...
fn update_streaming_partial_state(
    ...
    if preview_source != "held_display" {
        tracing::debug!(
            preview_source,
            "streaming AI rewrite quarantined for HUD truth single-chain v19; ASR HUD text remains the truth source"
        );
    }
...
fn schedule_post_commit_rewrite(
    runtime: &AppRuntime,
    committed_text: &str,
    delivery: OutputDelivery,
    source: &'static str,
) {
    let config = &runtime.config.voice.post_commit_rewrite;
    if !config.enabled {
        return;
    }
    ...
    let Some(ai_rewriter) = runtime.post_commit_rewriter.as_ref().cloned() else {
        tracing::info!(
            source,
            "post-commit AI rewrite skipped because client is unavailable"
        );
        return;
    };
    if config.require_native_replace && matches!(delivery, OutputDelivery::ClipboardOnly) {
        tracing::info!(
            source,
            ?delivery,
            "post-commit AI rewrite skipped because committed text was not inserted into target"
        );
        return;
    }
    if !try_acquire_post_commit_rewrite_slot(config.max_inflight.max(1)) {
        tracing::warn!(
            source,
            max_inflight = config.max_inflight.max(1),
            "post-commit AI rewrite skipped because inflight limit is reached"
        );
        return;
    }
    ...
    let ticket = match runtime
        .output_controller
        .capture_native_replace_ticket(committed_text)
    {
        Ok(Some(ticket)) => ticket,
        Ok(None) => {
            release_post_commit_rewrite_slot();
            tracing::info!(
                source,
                ?delivery,
                committed_text = %short_log_text(committed_text, 120),
                "post-commit AI rewrite skipped because no safe native replacement ticket was captured"
            );
            return;
        }
        Err(error) => {
            ...
            tracing::warn!(
                source,
                error = %error,
                "post-commit AI rewrite skipped because native replacement ticket capture failed"
            );
            return;
        }
    };

    let output_controller = Arc::clone(&runtime.output_controller);
    let context = runtime.output_controller.inspect_context_snapshot();
    let committed_text = committed_text.to_string();
    let delay = Duration::from_millis(config.delay_ms);
    let max_age = Duration::from_millis(config.max_age_ms.max(1));
    let max_output_chars = config.max_output_chars.max(8);
    tracing::info!(
        source,
        ?delivery,
        ...
        committed_text = %short_log_text(&committed_text, 160),
        "post-commit AI rewrite scheduled"
    );

    std::thread::spawn(move || {
        let _slot = PostCommitRewriteSlotGuard;
        if !delay.is_zero() {
            std::thread::sleep(delay);
        }
        if ticket.age() > max_age {
            tracing::info!(
                source,
                age_ms = ticket.age().as_millis(),
                max_age_ms = max_age.as_millis(),
                "post-commit AI rewrite aborted before request because ticket is too old"
            );
            return;
        }

        let rewrite_started_at = Instant::now();
        let response = match ai_rewriter.rewrite_committed_text(PostCommitRewriteRequest {
            ...
            Err(error) => {
                tracing::warn!(
                    source,
                    error = %error,
                    "post-commit AI rewrite request failed"
                );
                return;
            }
        };
        ...
        let Some(response) = response else {
            tracing::info!(source, "post-commit AI rewrite returned no replacement");
            return;
        };
        let Some(replacement_text) = sanitize_post_commit_rewrite_replacement(
            &committed_text,
        ...
        if ticket.age() > max_age {
            tracing::info!(
                ...
                age_ms = ticket.age().as_millis(),
                max_age_ms = max_age.as_millis(),
                "post-commit AI rewrite aborted after response because ticket is too old"
            );
            return;
        }

        match output_controller.replace_native_ticket(&ticket, &replacement_text) {
            Ok(ainput_output::NativeReplaceOutcome::Replaced) => {
                tracing::info!(
                    source,
                    elapsed_ms = rewrite_started_at.elapsed().as_millis(),
                    original = %short_log_text(&committed_text, 160),
                    replacement = %short_log_text(&replacement_text, 160),
                    "post-commit AI rewrite replaced committed text in place"
                );
            }
            Ok(ainput_output::NativeReplaceOutcome::AbortedSafe { reason }) => {
                tracing::info!(
                    source,
                    reason = %reason,
                    original = %short_log_text(&committed_text, 120),
                    replacement = %short_log_text(&replacement_text, 120),
                    "post-commit AI rewrite did not mutate target"
                );
            }
            Err(error) => {
                tracing::warn!(
                    source,
                    error = %error,
                    "post-commit AI rewrite native replacement failed"
                );
            }
        }
    });
...
fn sanitize_post_commit_rewrite_replacement(
    original_text: &str,
    ...
    if replacement_text.is_empty() {
        tracing::info!("post-commit AI rewrite rejected empty replacement");
        return None;
    }
    let replacement_chars = replacement_text.chars().count();
    if replacement_chars > max_output_chars.max(8) {
        tracing::info!(
            replacement_chars,
            max_output_chars = max_output_chars.max(8),
            replacement = %short_log_text(replacement_text, 160),
            "post-commit AI rewrite rejected replacement because it is too long"
        );
        return None;
    }
    if replacement_text == original_text {
        tracing::info!("post-commit AI rewrite kept original text unchanged");
        return None;
    }

    Some(replacement_text.to_string())
}

fn try_acquire_post_commit_rewrite_slot(max_inflight: usize) -> bool {
    let max_inflight = max_inflight.max(1);
    let mut current = POST_COMMIT_REWRITE_INFLIGHT.load(Ordering::Relaxed);
    loop {
        if current >= max_inflight {
            return false;
        }
        match POST_COMMIT_REWRITE_INFLIGHT.compare_exchange(
            current,
            current + 1,
            Ordering::AcqRel,
            Ordering::Relaxed,
...
fn release_post_commit_rewrite_slot() {
    let _ =
        POST_COMMIT_REWRITE_INFLIGHT.fetch_update(Ordering::AcqRel, Ordering::Relaxed, |current| {
            current.checked_sub(1)
        });
}

struct PostCommitRewriteSlotGuard;

impl Drop for PostCommitRewriteSlotGuard {
    fn drop(&mut self) {
        release_post_commit_rewrite_slot();
    }
}
...
fn repair_final_candidate_tail_overlap(
    ...
    .max(longest_edit_suffix_prefix_overlap_chars(
        display,
        appended_tail,
        1,
    ));
...
fn longest_edit_suffix_prefix_overlap_chars(prefix: &str, suffix: &str, max_edits: usize) -> usize {
    let prefix_chars = prefix.chars().collect::<Vec<_>>();
    let suffix_chars = suffix.chars().collect::<Vec<_>>();
    let max_overlap = prefix_chars
        .len()
        .min(STREAMING_FINAL_FUZZY_TAIL_OVERLAP_MAX_CHARS.max(1));
    for overlap in (STREAMING_FINAL_FUZZY_TAIL_OVERLAP_MIN_CHARS..=max_overlap).rev() {
        let min_suffix_len = overlap;
        let max_suffix_len = overlap
            .saturating_add(max_edits)
            .min(suffix_chars.len())
            .min(STREAMING_FINAL_FUZZY_TAIL_OVERLAP_MAX_CHARS.saturating_add(max_edits));
        for suffix_len in (min_suffix_len..=max_suffix_len).rev() {
            if suffix_len < STREAMING_FINAL_FUZZY_TAIL_OVERLAP_MIN_CHARS {
                continue;
            }
            let prefix_start = prefix_chars.len() - overlap;
            if edit_distance_with_limit(
                &prefix_chars[prefix_start..],
...
fn edit_distance_with_limit(left: &[char], right: &[char], limit: usize) -> usize {
...
fn prepare_streaming_preview_text(
    ...
    if rewrite_enabled {
        apply_streaming_punctuation_content_safe(&normalized, punctuator, false)
    } else {
        apply_streaming_punctuation_cleanup(&normalized)
    }
}
...
fn apply_streaming_punctuation(
    ...
    let punctuated_latest =
        sanitize_streaming_generated_punctuation(target_text, &punctuated_latest, finalize);
    ...
    apply_streaming_punctuation_cleanup(&format!("{}{}", frozen_prefix, punctuated_latest))
}
...
fn apply_streaming_punctuation_content_safe(
    ...
    if punctuated_content_chars < normalized_content_chars {
        ...
        apply_streaming_punctuation_cleanup(&punctuated)
    }
}
...
fn apply_streaming_punctuation_cleanup(text: &str) -> String {
    dedupe_streaming_punctuation(text)
}

fn sanitize_streaming_generated_punctuation(
    source_text: &str,
    punctuated_text: &str,
    finalize: bool,
) -> String {
    let source = source_text.trim();
    let punctuated = punctuated_text.trim();
    if source.is_empty() || punctuated.is_empty() {
        return String::new();
    }

    let source_content = content_text_without_sentence_punctuation(source);
    if source_content != content_text_without_sentence_punctuation(punctuated) {
        tracing::debug!(
            source = %short_log_text(source, 120),
            punctuated = %short_log_text(punctuated, 120),
            "streaming punctuation rejected because content alignment changed"
        );
        return apply_streaming_punctuation_cleanup(source);
    }

    let source_marks = punctuation_marks_by_content_index(source);
    let total_content_chars = source_content.chars().count();
    let mut content_index = 0usize;
    let mut last_boundary_content_index = 0usize;
    let mut result = String::with_capacity(punctuated.len());

    for ch in punctuated.chars() {
        if is_sentence_punctuation(ch) {
            if source_has_punctuation_mark(&source_marks, content_index, ch) {
                let mark = normalize_explicit_source_punctuation(ch);
                result.push(mark);
                last_boundary_content_index = content_index;
                continue;
            }

            if let Some(mark) = safe_generated_streaming_punctuation(
                ch,
                content_index,
                last_boundary_content_index,
                total_content_chars,
                finalize,
            ) {
                result.push(mark);
    ...
    apply_streaming_punctuation_cleanup(&result)
}

fn punctuation_marks_by_content_index(text: &str) -> Vec<(usize, char)> {
    let mut marks = Vec::new();
    let mut content_index = 0usize;
    for ch in text.chars() {
        if is_sentence_punctuation(ch) {
            marks.push((content_index, normalize_explicit_source_punctuation(ch)));
...
fn source_has_punctuation_mark(
    source_marks: &[(usize, char)],
    content_index: usize,
    ch: char,
) -> bool {
    let normalized = normalize_explicit_source_punctuation(ch);
    source_marks
        .iter()
        .any(|(index, mark)| *index == content_index && *mark == normalized)
}

fn normalize_explicit_source_punctuation(ch: char) -> char {
    match ch {
        ',' | '，' => '，',
        '.' | '。' | '．' => '。',
        '!' | '！' => '！',
...
fn safe_generated_streaming_punctuation(
    ch: char,
    content_index: usize,
    last_boundary_content_index: usize,
    total_content_chars: usize,
    finalize: bool,
) -> Option<char> {
    let terminal = content_index >= total_content_chars;
    match normalize_explicit_source_punctuation(ch) {
        '，' | '、' => can_accept_generated_internal_boundary(
            content_index,
            last_boundary_content_index,
            total_content_chars,
            4,
...
fn can_accept_generated_internal_boundary(
    content_index: usize,
    last_boundary_content_index: usize,
    total_content_chars: usize,
    min_left_chars: usize,
    min_right_chars: usize,
) -> bool {
    if content_index == 0 || content_index >= total_content_chars {
        return false;
    }
    let left_span = content_index.saturating_sub(last_boundary_content_index);
    let right_span = total_content_chars.saturating_sub(content_index);
    left_span >= min_left_chars && right_span >= min_right_chars
}
...
fn should_keep_streaming_preview_terminal_punctuation(
    ...
    let source_content_chars = content_chars_without_sentence_punctuation(source);
    punctuation_marks_by_content_index(source)
        .iter()
        .any(|(index, mark)| *index == source_content_chars && *mark == terminal)
}
...
mod tests {
    ...
    #[test]
    fn qwen_session_partial_truth_detects_existing_partial_updates() {
        let mut session = QwenSidecarSession::new(
            16_000,
            QwenStartResponse {
                session_id: "test-qwen".to_string(),
        ...
        let _ = apply_qwen_sidecar_partial_update(&mut session, "现在怎么又不行了", "qwen_partial")
            .expect("partial should be accepted");
        assert!(qwen_session_has_partial_truth(&session));
    }

    #[test]
    fn qwen_session_partial_truth_detects_existing_display_text() {
        let mut session = QwenSidecarSession::new(
            16_000,
            QwenStartResponse {
                session_id: "test-qwen".to_string(),
    ...
    #[test]
    fn streaming_cleanup_does_not_insert_leading_marker_commas() {
        assert_eq!(
            apply_streaming_punctuation_cleanup("另外这个加标点符号的逻辑"),
            "另外这个加标点符号的逻辑"
        );
    }

    #[test]
    fn streaming_cleanup_does_not_insert_mid_sentence_marker_commas() {
        assert_eq!(
            apply_streaming_punctuation_cleanup("最后一个字的问题还是可能会漏字尤其是语气词"),
            "最后一个字的问题还是可能会漏字尤其是语气词"
        );
    }

    #[test]
    fn streaming_cleanup_does_not_insert_project_specific_sentence_boundaries() {
        assert_eq!(
            apply_streaming_punctuation_cleanup(
    ...
    #[test]
    fn streaming_punctuation_sanitizer_downgrades_unanchored_question_marks() {
        assert_eq!(
            sanitize_streaming_generated_punctuation("这个怎么回事啊", "这个怎么？回事啊？", true),
            "这个怎么回事啊。"
        );
    }

    #[test]
    fn streaming_punctuation_sanitizer_strips_unanchored_preview_question_marks() {
        assert_eq!(
            sanitize_streaming_generated_punctuation("这个怎么回事啊", "这个怎么？回事啊？", false),
            "这个怎么回事啊"
        );
    }

    #[test]
    fn streaming_punctuation_sanitizer_keeps_source_anchored_question_marks() {
        assert_eq!(
            sanitize_streaming_generated_punctuation("这个功能可用吗？", "这个功能可用吗？", true),
            "这个功能可用吗？"
        );
    }
    ...
    #[test]
    fn final_commit_repairs_user_failed_inserted_tail_overlap() {
        let selected = select_streaming_commit_text(
            "\u{7136}\u{540e}\u{ff0c}\u{4f60}\u{5b9e}\u{6d4b}\u{7684}\u{65f6}\u{5019}\u{4e5f}\u{6211}\u{521a}\u{624d}\u{5931}\u{8d25}\u{6848}\u{4f8b}\u{6765}\u{5b9e}",
            "\u{7136}\u{540e}\u{ff0c}\u{4f60}\u{5b9e}\u{6d4b}\u{7684}\u{65f6}\u{5019}\u{4e5f}\u{6211}\u{521a}\u{624d}\u{5931}\u{8d25}\u{6848}\u{4f8b}\u{6765}\u{5b9e}\u{5931}\u{8d25}\u{7684}\u{6848}\u{4f8b}\u{6765}\u{5b9e}\u{6d4b}\u{3002}",
            StreamingCommitSource::OfflineFinal,
        );
        assert_eq!(selected.source, StreamingCommitSource::OfflineFinal);
        assert_eq!(
            selected.text,
            "\u{7136}\u{540e}\u{ff0c}\u{4f60}\u{5b9e}\u{6d4b}\u{7684}\u{65f6}\u{5019}\u{4e5f}\u{6211}\u{521a}\u{624d}\u{5931}\u{8d25}\u{7684}\u{6848}\u{4f8b}\u{6765}\u{5b9e}\u{6d4b}\u{3002}"
        );
        assert!(!selected.text.contains("\u{6848}\u{4f8b}\u{6765}\u{5b9e}\u{5931}\u{8d25}\u{7684}\u{6848}\u{4f8b}\u{6765}\u{5b9e}"));
    }
    #[test]
    ...
    fn output_text_repairs_observed_i_and_punctuation_artifacts() {
        ...
        assert_eq!(prepared, "简直就是灾难的标点符号都不对。");
